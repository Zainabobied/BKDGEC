...
BKDGEC/
│
├── images and diagrams/ # Visual diagrams and figures used in the paper & README
│
├── synthetic training data/ # Synthetic parallel data generated by EDSE
│
├── trained models/ # Saved checkpoints (Teacher L2R, Student R2L, Fine-tuned models)
│
├── system outputs/ # Model inference outputs (validation/test predictions)
│
├── m2Scripts/ # Scripts for official M² scorer evaluation (MaxMatch)
│
├── configs/ # Model & training configuration files (JSON)
│
├── scripts/ # Utility scripts (tokenizer training, evaluation, data processing, etc.)
│
